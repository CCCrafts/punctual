...
- **Event types** with buffers, notice windows, horizons, daily caps, custom
  questions
- **Teams** — round-robin and collective scheduling with required and
  optional hosts, each on their own schedule; team admins manage members,
  event types and each member's availability, and can add or swap hosts
  on a booking after the fact
- **Calendar events that say who the meeting is with** — every
  participant named with their company, on the hosts' calendars and in
