...
export interface AvailabilityRepository {
  ...
  /** `schedule.id` is pre-generated by the caller (same convention as `EventTypeRepository.create`). */
  create(userId: string, schedule: Schedule): Promise<Schedule>
  update(userId: string, scheduleId: string, patch: Partial<Omit<Schedule, 'id' | 'userId' | 'isDefault'>>): Promise<void>
  /**
