...
export const eventTypes = sqliteTable(
  'event_types',
  {
    ...
    createdAt: integer('created_at').notNull(),
    // Null = the owner's default schedule (CCC-581) — only meaningful for a
...
/**
 * Superseded by `schedules` below (CCC-581: a host can have more than one
...
/**
 * A host's named schedules (CCC-581) — 1:N where `availability_schedules`
